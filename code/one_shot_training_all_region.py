...
#  Last Modified: Thu Jan 10 19:36:56 2019
#
...
parser.add_argument('--source_name', default='mouse-brain-P56-sice-3-21-atlas_cropped', help='Name of the source image')
...
parser.add_argument('--augmentation_num', default=20, type=int, help='number of augmentated images per epoch')
...
region_list = []
suffix = args.source_name + '_{}'.format(args.loss) + '_region-all' + '_lr_{:1.1e}'.format(args.learning_rate)
...
for i in range(len(test_sequence)):
    ...
    transform_matrix = np.load("{}/95_transform_matrix/{}.npy".format(args.data_dir, test_sequence.file_name_list[i]))
    ...
    for idx, region_idx in enumerate(region_list):
        ...
        new_output = cv2.resize(new_output, (original_img.shape[1], original_img.shape[0]))
        if new_index_output is None:
            new_index_output = np.zeros((original_img.shape[0], original_img.shape[1]), dtype=np.uint8)
