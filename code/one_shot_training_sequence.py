    ...
    def __getitem__(self, idx):
        if self.no_ref_mask:
            ...
            if self.use_background: 
                ...
                batch_x = np.zeros(shape=(self.batch_size, self.image_width, self.image_height, self.channel_number+1), dtype=np.float32)
        # label
        ...
        for i in range(self.batch_size):
            ...
            if self.use_background:
                ...
                transformed_index_mask = np.zeros((self.image_height, self.image_width), dtype = np.uint8)
