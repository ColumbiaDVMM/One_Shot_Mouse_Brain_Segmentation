...
class OneShotValidationSequenceMultiRegion(Sequence):
    ...
    def load_image_and_mask(self, image_dir, mask_dir):
        ...
        for img_name in glob.glob(image_dir + '/*.png'):
            ...
            im_gray = cv2.resize(im_gray, (self.image_width, self.image_height))
            # aug_image = cv2.equalizeHist(im_gray)
            aug_image = im_gray
            image_list.append(aug_image)
            if not self.no_mask:
                if self.use_background:
                    multi_channel_mask = np.zeros((self.image_height, self.image_width, len(self.region_list) + 1), dtype = np.uint8)
                    index_mask = np.zeros((self.image_width, self.image_height), dtype = np.uint8)
                    for idx, region_index in enumerate(self.region_list):
                        mask = cv2.imread(mask_dir + '/{}-{}.png'.format(pure_name, region_index), cv2.IMREAD_GRAYSCALE)
                    ...
                    multi_channel_mask = np.zeros((self.image_height, self.image_width, len(self.region_list)), dtype = np.uint8)
                    for idx, region_index in enumerate(self.region_list):
                        mask = cv2.imread(mask_dir + '/{}-{}.png'.format(pure_name, region_index), cv2.IMREAD_GRAYSCALE)
                        mask = cv2.resize(mask, (self.image_height, self.image_width), interpolation=cv2.INTER_NEAREST)
    ...
    def __getitem__(self, idx):
        ...
        if self.no_ref_mask:
            batch_x = np.zeros(shape=(self.batch_size, self.image_width, self.image_height, 1), dtype=np.float32)
        else:
            batch_x = np.zeros(shape=(self.batch_size, self.image_width, self.image_height, self.channel_number), dtype=np.float32)

        if not self.no_mask:
            if self.use_background:
                batch_y = np.zeros(shape=(self.batch_size, self.image_width, self.image_height, self.channel_number), dtype=np.uint8)
            else:
                batch_y = np.zeros(shape=(self.batch_size, self.image_width, self.image_height, self.channel_number-1), dtype=np.uint8)
