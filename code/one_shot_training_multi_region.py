...
parser.add_argument('--source_name', default='P4_100033295_317_r', help='Name of the source image')
parser.add_argument('--data_dir', default='../data/', help='folder to image data')
parser.add_argument('--log_dir', default='../multi_region/unet_log/', help='folder to output log')
parser.add_argument('--model_dir', default='../multi_region/model/', help='folder to save model')
parser.add_argument('--output_dir', default='../multi_region/result/', help='folder to draw result')

parser.add_argument('--unet_depth', default=6, type=int, help='depth for unet')
parser.add_argument('--start_filters', default=8, type=int, help='starting hidden channel number')
...
parser.add_argument('--augmentation_num', default=20, type=int, help='number of augmentation images per epoch')
parser.add_argument('--region_list', default="1-2-3-4", type=str, help='region used for training')
...
region_list = []
suffix = args.source_name + '_{}'.format(args.loss) + '_region-{}'.format(args.region_list)
...
if args.use_background:
    training_mask = np.zeros((image_size[0], image_size[1], len(region_list)+1), dtype = np.uint8)
else:
    training_mask = np.zeros((image_size[0], image_size[1], len(region_list)), dtype = np.uint8)

# no back ground ref channel
ref_mask = np.zeros((image_size[0], image_size[1], len(region_list)), dtype = np.float32)

# get background mask
background_mask = np.zeros((image_size[0], image_size[1]), dtype = np.uint8)+255
for idx, region_idx in enumerate(region_list):
    ...
    try:
        ...
        os.stat('{}/ref_mask/{}-{}.png'.format(args.data_dir, args.source_name, region_idx))
        ...
        expand_training_mask = utils.expandImage(tmp_mask.copy(), expand_pixel=25)
...
if args.no_alignment:
    ...
    test_mask_dir = '{}/mask/all_masks_aligned/{}/'.format(args.data_dir,args.source_name)
...
if args.idx == 0:
    csv_file = csv.writer(open('{}/csv_file/{}.csv'.format(args.output_dir, suffix, suffix), 'w'), delimiter=',')
else:
    csv_file = csv.writer(open('{}/csv_file/{}.csv'.format(args.output_dir, suffix, suffix), 'a'), delimiter=',')
...
for i in range(len(test_sequence)):
    ...
    if not args.use_background:
        background_channel = np.ones((np.shape(output)[0], np.shape(output)[1], 1)) * 0.5
    ...
    transform_matrix = np.load("{}/transform_matrix/{}/{}.npy".format(args.data_dir, args.source_name, test_sequence.file_name_list[i]))
    ...
    for idx, region_idx in enumerate(region_list):
        ...
        new_output = cv2.resize(new_output, (original_mask.shape[1], original_mask.shape[0]))
        if new_index_output is None:
            new_index_output = np.zeros((original_mask.shape[0], original_mask.shape[1]), dtype=np.uint8)
...
for i in range(len(region_list)):
    ...
    tem_list = ['{:.3f}'.format(x) for x in tem_list] + ['{:.3f}'.format(np.mean(tem_list))]
